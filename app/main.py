...
BASE_DIR = os.path.dirname(os.path.abspath(__file__))  # Get absolute path of current directory
DB_PATH = os.path.join(BASE_DIR, '../db/food_distribution.db')  # Ensure it goes up one level
...
def customers():
    try:
        ...
        if street_name:
            query += " AND StreetName LIKE :street_name"
            params['street_name'] = f"%{street_name}%"
        ...
        return render_template('customers.html', customers=result, street_name=street_name, district=district)
...
def add_customer():
    ...
    language = session.get('language', 'en')
    print(f"Current language: {language}")  # Check the console for this output
...
def search_customer():
    if request.method == 'POST':
        ...
        # Dynamically build the query based on search criteria
        if search_criteria == "Store Name":
            results = Customer.query.filter(Customer.StoreName.like(f"%{search_query}%")).all()
        elif search_criteria == "District":
            results = Customer.query.filter(Customer.District.like(f"%{search_query}%")).all()
        elif search_criteria == "Street Name":
            results = Customer.query.filter(Customer.StreetName.like(f"%{search_query}%")).all()
        else:
            results = []

        return render_template('search_results.html', results=results, query=search_query, criteria=search_criteria)
...
@login_required
def add_order(customer_id=None):  # Make customer_id optional
    ...
    customer = Customer.query.get(customer_id) if customer_id else None
    customers = Customer.query.all() if not customer else []  # Provide customer list for selection if no customer_id
    print(customers)
    if request.method == 'POST':
        try:
...
#     products = Product.query.all()
#     customer = Customer.query.get(customer_id) if customer_id else None
#     customers = Customer.query.all() if not customer else []
#
#     if request.method == 'POST':
#         try:
#             # If no customer is pre-selected, get customer ID from the form
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            if role == 'sales':
                db.session.execute(
                    text("INSERT INTO SalesReps (UserID, MonthlyTarget, CommissionRate) VALUES (:user_id, :target, :rate)"),
                    {
                        "user_id": user['UserID'],
...
@login_required
@role_required(['admin'])  # Restrict access to admins
def edit_order(order_id):
    # Fetch the order and its details
    order = Order.query.get_or_404(order_id)
    order_details = OrderDetail.query.filter_by(OrderID=order_id).all()
    products = Product.query.all()  # Fetch all available products
    ...
    if request.method == 'POST':
        try:
            ...
            # Update inventory: reverse changes from the current order
            for detail in order_details:
                batches = Batch.query.filter_by(ProductID=detail.ProductID).order_by(Batch.PurchaseDate.desc()).all()
                remaining_to_add = detail.Quantity
                for batch in batches:
                    if batch.Quantity + remaining_to_add <= batch.PurchasePrice:
                        batch.Quantity += remaining_to_add
                        break
                    else:
                        remaining_to_add -= batch.PurchasePrice - batch.Quantity
...
def assign_stock():
    ...
    if request.method == 'POST':
        try:
            ...
            ).filter_by(ProductID=product_id).scalar() or 0
...
@login_required
@role_required(['admin'])  # Restrict this route to admin users only
...
def add_product():
    if request.method == 'POST':
        # Safely handle form data after confirming it's a POST request
        ...
        new_product = Product(ProductName=product_name, BasePrice=base_price)
...
def edit_product(product_id):
    ...
    # Render the form on GET request, pre-filled with the current product details
